// Mitochondrial Nucleoid Analysis Macro - Final Fixed Version
// Analyzes 4-channel images: CP (C1), dP (C2), T20 (C3), TFAM (C4)

macro "Mitochondrial Nucleoid Analysis Final Fixed" {
    ...
    // Step 4: Create T20 mask and apply to CP
    selectWindow("T20_processed");
    ...
    // Apply mask to CP - KEEP INTENSITY VALUES FOR SPOT DETECTION
    selectWindow("CP_processed");
    ...
    // Apply TF mask to the intensity-preserved CP image
    selectWindow("T20_mask_overlay_CP_for_spots");
    run("Duplicate...", "title=TF_T20_mask_overlay_CP_for_spots");
    run("32-bit");
    selectWindow("TF_mask");
    run("Duplicate...", "title=TF_mask_for_overlay");
    run("32-bit");
    run("Divide...", "value=255");
    imageCalculator("Multiply", "TF_T20_mask_overlay_CP_for_spots", "TF_mask_for_overlay");
    selectWindow("TF_T20_mask_overlay_CP_for_spots");
    
    // Create a version for saving
    run("Duplicate...", "title=TF_T20_mask_overlay_CP");
    run("32-bit");
    saveAs("Tiff", output_dir + "TF_T20_mask_overlay_CP.tif");
    ...
    if (selectionType() == 10) { // Point selection type
        ...
        cp_spots_in_t20 = xpoints.length;
    }
    ...
    if (indexOf(current_image, "Maxima") >= 0 || indexOf(current_image, "T20_mask_overlay_CP_for_spots") >= 0) {
        rename("CP_spots_in_T20_detected");
        saveAs("Tiff", output_dir + "CP_spots_in_T20_detected.tif");
    }
    
    // Count CP spots within both T20 and TF masks using the intensity-preserved image
    run("Clear Results");
    selectWindow("TF_T20_mask_overlay_CP_for_spots");
    run("Find Maxima...", "prominence=15 output=[Point Selection]");
    
    cp_spots_in_t20_tf = 0;
    if (selectionType() == 10) { // Point selection type
        getSelectionCoordinates(xpoints2, ypoints2);
        cp_spots_in_t20_tf = xpoints2.length;
    }
    print("CP Spots within both T20 and TF masks: " + cp_spots_in_t20_tf);
    
    // Create visualization
    selectWindow("TF_T20_mask_overlay_CP_for_spots");
    run("Find Maxima...", "prominence=15 output=[Maxima Within Tolerance]");
    current_image2 = getTitle();
    if (indexOf(current_image2, "Maxima") >= 0 || indexOf(current_image2, "TF_T20_mask_overlay_CP_for_spots") >= 0) {
        rename("CP_spots_in_T20_TF_detected");
        saveAs("Tiff", output_dir + "CP_spots_in_T20_TF_detected.tif");
    }
    
    // Calculate percentage
    if (cp_spots_in_t20 > 0) {
        cp_in_tf_percentage = (cp_spots_in_t20_tf / cp_spots_in_t20) * 100;
        print("Percentage of CP spots (within T20) that are also within TF mask: " + cp_in_tf_percentage + "%");
    } else {
        print("Cannot calculate percentage (no CP spots within T20 mask)");
    }
